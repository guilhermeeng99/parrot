...
- **Engine status** Card (read-only):
  - The **Engine status label** sources `GET /engine/status` → `{"active":"omnivoice","device":"<id>"}`. Render: a **Badge** "Engine: OmniVoice" + a device line "Running on **{device_label or device}**" mapping `cuda`/`mps`/`rocm`/`cpu` to friendly text ("NVIDIA GPU (CUDA)", "Apple GPU (Metal)", "AMD GPU (ROCm)", "CPU — slower but works"). One fixed engine, **no picker** ([settings.md Rule 1](./settings.md)).
