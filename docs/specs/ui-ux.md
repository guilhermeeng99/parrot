...
How Parrot's design system becomes screens. [design-system.md](./design-system.md) owns the tokens and the shared component recipes (the `@theme` block, the "Empower" dark command-center palette, the Svelte UI primitives under `frontend/src/lib/components/ui/`). This spec owns the *application of* those tokens: the app shell, the five screens, the core flow that ties them together, and the interaction states every async action must render. Behavior is owned by the feature specs and linked inline; this spec never re-specifies an endpoint contract.
...
**Scope locks (V1):** DARK THEME ONLY — a light-section rhythm / theme toggle is backlog. No UI-zoom feature — backlog. Any earlier "theme: light|dark default dark + zoom" model from `settings.md` / `design-system.md` is superseded by the Empower dark-only system. Three type families, clear roles: Poppins (`font-display`, headlines), Inter (`font-body`, default), Playfair Display (`font-serif`, optional). One action color: `button-yellow` (background/border/affordance only — never text). 8px grid. The process model (three processes, loopback `:3900`, supervisor-owned sidecar lifecycle) is [architecture.md](./architecture.md).

---

## 1 — App Shell & Layout

The window is a single Tauri WebView ([architecture.md §1](./architecture.md)). Inside it the layout is the Empower dark command-center shell, applied for Parrot.

### 1.1 — The shell

```
┌──────────────────────────────────────────────────────────────────────┐
│  HEADER (sticky, bg-night-sky/90 + backdrop-blur, border-b            │
│          border-white/10, h-16)                                        │
│  ┌──────────────────────── max-w-[1000px], px-6 ─────────────────────┐ │
│  │ 🦜 Parrot  [local]      ‹Clone › Speak › Settings›       v0.0.x / │ │
│  │ (display,bold)(Badge)        (Pill nav, ml-auto)     Update btn   │ │
│  └────────────────────────────────────────────────────────────────┘ │
├──────────────────────────────────────────────────────────────────────┤
│  MAIN  (body bg-deep-space)                                            │
│  ┌──────────────────────── max-w-[1000px], px-6, py-12 ─────────────┐ │
│  │  [optional centered screen header: title + one-line description] │ │
│  │                                                                  │ │
│  │  [ Card(s) — flex flex-col gap-6, rounded-3xl bg-charcoal-card   │ │
│  │    p-6 shadow-sm-2 ring-1 ring-white/5 ]                          │ │
│  └──────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────┘
```

- **Header** — sticky `top-0 z-50`, `border-b border-white/10 bg-night-sky/90 backdrop-blur`, inner row `h-16 max-w-[1000px] mx-auto px-6 flex items-center gap-4`.
...
- No UI-zoom control in V1. The OS/WebView's own zoom is untouched; honoring it is a backlog item alongside the light-section rhythm.
...
**Boot splash (supervisor-owned, pre-`/healthz`).** Shown while the Rust supervisor brings the sidecar up ([architecture.md §3.1](./architecture.md), [first-run-setup.md §5](./first-run-setup.md)). A single centered **Card** on `deep-space`:
- Logo + **Spinner** (`h-5 w-5 animate-spin rounded-full border-2 border-button-yellow/30 border-t-button-yellow`) + a stage line in `text-body-lg text-ash-gray`, mapping the supervisor state machine to plain copy:
...
- Body `text-body-lg text-ash-gray`: "Parrot needs to download its voice engine once (a few hundred MB). After this, everything runs offline — no account, no internet." Show the target path (`hf_cache_dir`) in muted `font-mono text-body` and the free-space line.
...
- **Progress** (the `downloading` state) replaces the button with the **ProgressBar** component:
  - `phase=resolving` (and the ~2s heartbeats) → the **indeterminate** ProgressBar (a chunk sliding across a `slate-fill` track, `button-yellow` fill) with "Preparing download…" — never a frozen 0%.
  - `phase=progress` → determinate ProgressBar driven by `pct` (0–1), with `{filename}` and a `rate` readout in `font-mono text-body text-ash-gray`, e.g. "model-00001-of-00002.safetensors · 142 MB / 380 MB · 6.4 MB/s".
  - `phase=install_retry` → keep the bar, show a non-blocking `ash-gray` line "Network hiccup — retrying (attempt {n})…" using the redacted error.
...
- **Record path** → the **Recorder** component: a large `button-yellow` round mic button (≥40px), a live elapsed timer in `font-mono`, and a level meter (animated `button-yellow` bar). States: idle → recording (button shows stop, pulsing ring honoring reduced-motion) → captured. On stop, the blob is held in memory and previewable.
- **Upload path** → the **Dropzone** component (DS `Dropzone` recipe): `border-2 border-dashed`, `over` → `border-button-yellow bg-slate-fill/60`, else `border-metal-gray bg-deep-space hover:border-button-yellow`. Copy: "Drop an audio file here, or click to choose" / hint `text-body text-ash-gray` "wav · mp3 · m4a · flac · ogg · webm — stays on your device". File selection uses the native Tauri dialog.
- **Captured preview** (shared by both paths): an inline **AudioPlayer** (§3) on the captured clip, a **"Re-record" / "Choose another"** secondary button, and a **length hint**:
  - `< ~3s` → `ash-gray` caution "That's quite short — a 3–10s clip clones more reliably." (warn, never block — [voice-cloning.md EDGE-1](./voice-cloning.md)).
  - `> ~20s` → `ash-gray` "Long clips are slower and clone less cleanly — 3–10s of clean speech is ideal." ([voice-cloning.md EDGE-2](./voice-cloning.md)).
- **Save form** (enabled only once a capture exists):
  ...
  - **Field** "What was said? (optional)" → a small **TextComposer**/textarea for `ref_text`, with helper `text-body text-ash-gray`: "Type the exact words in your clip — it sharpens the clone. Leave blank if unsure: **better empty than wrong.**" ([voice-cloning.md BR-4 / EDGE-8](./voice-cloning.md)).
...
**Library Card.** The voice library ([voice-profiles.md §4.1](./voice-profiles.md)) rendered as a responsive grid of **VoiceCard**s (`flex flex-wrap gap-6`, each VoiceCard = a `rounded-3xl bg-charcoal-card p-6 shadow-sm-2 ring-1 ring-white/5` Card, interactive → `hover:ring-button-yellow/40 hover:shadow-sm`):
- Each VoiceCard: voice name (`text-heading font-display tracking-tight text-cloud-whisper`), a **locked Badge** when `is_locked` ("Locked" pill, `cloud-whisper` on `slate-fill`, paired with a lock glyph so it never relies on color alone), created-at in `text-body text-ash-gray`, a mini **AudioPlayer** for the profile's representative clip (`GET /profiles/{id}/audio`), and a **"Speak with this"** quick action that jumps to Speak with the profile pre-selected.
...
- A meta row in `font-mono text-body text-ash-gray`: duration (`X-Audio-Duration`), generation time (`X-Gen-Time`), and seed (`X-Seed`) when present — the data that lets a user reproduce a take.
...
Owner: [voice-profiles.md](./voice-profiles.md). Opened from a VoiceCard. A single **Card** (modal sheet or routed sub-page; either way `rounded-3xl bg-charcoal-card p-6 shadow-sm-2 ring-1 ring-white/5`) for one profile.

- **Header row**: editable name (inline-editable `text-heading`, or a "Rename" affordance opening a text input), the **locked Badge** (with lock glyph) reflecting `is_locked`, and created-at in `text-body text-ash-gray`.
...
- **Anatomy**: a play/pause button (≥40px, `button-yellow`), a scrub track (the DS slider track styling — `slate-fill` track, `button-yellow` fill, 18px thumb), a `font-mono` time readout (`0:03 / 0:08`), and an optional **Download** icon-button (native save dialog). A "busy" variant shows a Spinner over the play button while bytes are still streaming.
...
| **loading** | The triggering control is **disabled** and shows a **Spinner** + present-tense label ("Saving…", "Generating…", "Downloading…"). Long indeterminate work uses the **ProgressBar** (indeterminate chunk) or, when `pct` is known, a determinate bar. | `Spinner` (`border-button-yellow/30 border-t-button-yellow`), `ProgressBar`, `disabled:opacity-50`. | Never a frozen 0%: `resolving` heartbeats drive the indeterminate bar ([first-run-setup.md §4](./first-run-setup.md)). The page stays interactive where the work is off-thread ([synthesis.md BR-2](./synthesis.md)). |
| **empty** | A friendly, instructive empty block — never styled as an error. | `text-body-lg text-ash-gray`, centered, inside the relevant Card. | Empty library → "clone your first voice"; empty history → "nothing spoken yet". |
| **success** | A brief confirmation: a toast and/or arriving on the working surface; success-colored where a badge is warranted. | `--color-success` (`#5fd39a`) for badges/checks; toast. | Setup success = landing on Clone (no toast needed). Save/lock/delete = toast. Paired with text/icon, never color alone. |
| **error** | A **danger**-colored, plain-language message that says **what happened and what to do next**, plus the actionable control (Retry / Flush & retry / Upload instead / Free space). The underlying technical detail is available but secondary (mono panel or "view log"). | `--color-danger` (`#ff6b5e`) for the heading/border; PrimaryButton or outline button for the recovery action; `font-mono` for the raw detail. | Cooldown-aware retries show "Retry in {n}s" ([first-run-setup.md Rule 8](./first-run-setup.md)). Connection-refused is "engine starting", not a hard error ([architecture.md §4](./architecture.md)). Tokens/paths are redacted/`~`-stripped per [first-run-setup.md §2](./first-run-setup.md) and [../../CLAUDE.md](../../CLAUDE.md). |
...
- **Simple, fast, token-bound.** Hover lifts and color shifts use the DS button/card transitions (`transition`, `hover:brightness-105` on PrimaryButton; `transition hover:ring-button-yellow/40 hover:shadow-sm` on interactive Cards). No bespoke easings or large translate animations.
...
- **Focus rings everywhere.** Every interactive element carries the shared `focusRing` (`focus-visible:ring-2 focus-visible:ring-button-yellow focus-visible:ring-offset-2 focus-visible:ring-offset-deep-space`). Visible, `button-yellow`, 2px, with a `deep-space` offset.
...
- [../ROADMAP.md](../ROADMAP.md) — milestone context (the light-section rhythm and UI-zoom are backlog).
