...
1. **Create requires a name and a supported reference clip.** `POST /profiles` rejects a missing/blank `name` (`400`) and an upload whose extension is not in the accepted set `{.wav, .mp3, .m4a, .flac, .ogg, .webm}` (`415`, naming the offending extension). The accepted set covers every in-app upload plus the recorder's `.webm`. The uploaded audio is written to `parrot_data/voices/{id}{ext}`, where `ext` is taken from the upload's original filename (defaulting to `.wav`). Create validates the **extension only** — it has no audio decoder, so a corrupt or all-silent clip is not caught here; that surfaces later at synthesis time (see Rule 2 and [synthesis.md](./synthesis.md)).
2. **Decode and silence checks are deferred to synthesis.** The light create path carries no audio decoder (torchaudio + the ffmpeg fallback live in the `engine` extra, loaded lazily only at inference). So an unreadable or all-silent reference is **not** rejected at create — it is caught the first time the profile is used to synthesize, surfacing as a synthesis error (`500`), not a create-time `422`. See [synthesis.md](./synthesis.md).
3. **Create is atomic against orphans.** If the audio file is written but the DB insert fails, the just-written file is deleted before the error propagates. A failed create leaves no file and no row.
4. **Read by id is the source of truth.** `GET /profiles/{id}` returns the full row. A missing id is a `404` with a message that names the likely cause (deleted from another tab/window).
...
- **Errors:** `400` missing/blank `name`; `415` unsupported audio extension (not in `{.wav, .mp3, .m4a, .flac, .ogg, .webm}` — the message names the offending extension); on DB-insert failure the orphan audio file is removed and the error propagates as `500`.
- **Validation scope:** create checks the **extension only** — it has no audio decoder. A corrupt or all-silent clip is *not* rejected here; full decode + the all-silence check run at synthesis time and surface as a synthesis error (`500`). There is **no** create-time `422` for silent/undecodable audio. See [synthesis.md](./synthesis.md).
...
- **Create with a non-`.wav` upload.** Any extension in the accepted set `{.wav, .mp3, .m4a, .flac, .ogg, .webm}` is taken from the uploaded filename and preserved in `ref_audio_path` (the recorder's `.webm` and all in-app uploads are accepted); a filename with no extension defaults to `.wav`.
- **Create with an unsupported extension.** An upload whose extension is outside the accepted set is rejected `415`, with the offending extension named in the message; no file is written and no row is inserted.
- **Create with a corrupt or all-silent reference.** Not caught at create — the light create path has no decoder, so the row and file are written. The failure surfaces the first time the profile is used to synthesize, as a synthesis error (`500`), not a create-time `422`. See [synthesis.md](./synthesis.md).
