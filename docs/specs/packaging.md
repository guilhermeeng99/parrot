...
Parrot is an independent, Apache-2.0 open-source app — **not** a code fork of OmniVoice Studio. It reuses **only** the Apache-2.0 `omnivoice` model library for inference and reimplements its own app from these specs; OmniVoice's FSL-1.1-ALv2 app code is a design reference only, never copied (see [../LICENSING.md](../LICENSING.md)). Scoped to just clone-and-speak, it ships none of OmniVoice's other features (dubbing, general ASR/dictation, the gallery, batch, the multi-engine picker, the C++ GGUF backend) — the one narrow exception is reference-clip transcription to fill `ref_text` (see [transcription.md](./transcription.md)), which adds `openai-whisper` + `av` to the engine extra but no installer binary. The packaging surface is correspondingly small — one engine (`omnivoice`, pure-Python via `transformers`), no C++ binary, a smaller installer.
...
BundleConfig (frontend/src-tauri/tauri.conf.json → "bundle")
  ...
  externalBin       : ["binaries/uv.exe"]        # Phase-3; ffmpeg/ffprobe DROPPED (PyAV supplies
                                                 #   ffmpeg in-wheel for ASR decode; no dub/media path)
  resources         : ["../../sidecar/pyproject.toml",
                       "../../sidecar/uv.lock",
                       "../../README.md",
                       "../../sidecar"]           # the Python engine source tree (was "backend"+"omnivoice")
  icon              : [32, 128, 128@2x, ico]   # .icns dropped — Windows-only bundle
...
UpdaterConfig (tauri.conf.json → "plugins.updater")
  ...
  pubkey            : <Parrot's own minisign public key>   # real key, committed; NOT OmniVoice's
...
- the version is auto-bumped on every push to main (release.yml), declared in 4 manifests that must stay in lockstep:
  ...
  bundle.version == git tag is CI-enforced).
...
4. **First-launch venv bootstrap is idempotent and host-resolved.** On first run with no usable venv, the supervisor runs `uv venv` (Python 3.11+) then `uv sync --no-dev --extra engine` against the bundled `uv.lock`. `--no-dev` keeps the test-only `dev` group (pytest/httpx) out of the shipped runtime venv; `--extra engine` pulls the ML stack (the `omnivoice` model lib + torch/torchaudio/transformers/pedalboard, plus `openai-whisper` + `av` for reference transcription — see [transcription.md §8](./transcription.md)) that lives in the `engine` optional-dependency group — kept out of the default/test sync (and this repo's CI) because it is multi-GB and the model boundary is mocked in tests. **Note:** `openai-whisper` ships as an sdist (no PyPI wheel), so `uv sync --extra engine` builds it from source at first-run bootstrap; it is pure-Python (setuptools build, no C extension), so the engine sync must complete that build step — exercise it on a clean Windows 10/11 box as part of the release gate, since the headless smoke test syncs without the engine extra. The Apache-2.0 `omnivoice` model lib is a **PyPI dependency** in the `engine` extra (pinned in `uv.lock`), imported lazily by `model_manager` via the engine adapter (`from omnivoice import OmniVoice`; see [../LICENSING.md](../LICENSING.md)). A completed venv is detected and reused on subsequent launches; the engine is only started after the venv exists and `GET /healthz` returns healthy. The install (`uv venv` + `uv sync`) and run (start → poll health) logic lives in the Rust sidecar supervisor and runs invisibly inside the Tauri process — there are no standalone `scripts/install.sh`/`scripts/run.sh` helpers. The bootstrapped venv location is stated identically in [architecture.md](./architecture.md).
5. **`ffmpeg`/`ffprobe` are not shipped.** OmniVoice bundled them as `externalBin` for video dubbing and demucs. Parrot has no dubbing/media-extraction path, and its one ASR use — reference-clip transcription — decodes audio via **PyAV (`av`)**, whose wheel bundles the ffmpeg libraries (see [transcription.md §3](./transcription.md)). So the standalone `ffmpeg`/`ffprobe` binaries are still removed from `externalBin`, shrinking every installer.
6. **CUDA/NVIDIA wheels are never *bundled* in the installer — they are downloaded at first run, and only on NVIDIA machines.** No torch or CUDA wheels ship inside the MSI; that (with the model weights, Rule 3) is what keeps the installer under the GitHub Releases asset cap. Instead the first-run venv bootstrap (Rule 4) selects the torch wheel variant from NVIDIA detection: a GPU box pulls the CUDA wheels (`torch+cu124` plus the `nvidia-*-cu12` runtime libs) from PyTorch's `cu124` index via `--extra cu124`; every other machine pulls the CPU wheel via `--extra cpu`. `triton`/`flash_attn` are not required by the engine and are not installed. So GPU acceleration is real and automatic on NVIDIA hardware, while CPU-only machines never download the multi-GB CUDA stack they can't use. The mechanism (mutually-exclusive uv extras + the `nvidia-smi` probe) is specified in [device-detection.md](./device-detection.md) Rule 10.
7. **The auto-updater points at Parrot's own releases.** `plugins.updater.endpoints` and `plugins.updater.pubkey` point at Parrot's GitHub repo and Parrot's own minisign keypair: the real keypair has been generated, the public key is committed in `tauri.conf.json`, and the private key (plus password) lives in the CI secrets `TAURI_SIGNING_PRIVATE_KEY(_PASSWORD)`. Shipping with OmniVoice's endpoint or pubkey would be a release-blocking bug — it would pull OmniVoice's `latest.json` and could not verify Parrot's signatures.
